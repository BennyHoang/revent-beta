...
class bp_profile_cover_functions{
			...
			if ( !isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'],'bp_cover_upload') ){
				...
				if(! empty( $_FILES['profile_cover']['name'])){
					...
					if ( ! empty( $file['error'] ) ) {
						...
						$this->message =  '<div class="success message">'.__('Coverbilde lastet opp. ','vibe').'</div>';
					}
	...
	function bp_profile_cover_page_content() {
		...
		?>
		<h4><?php _e( 'Endre coverbilde', 'buddypress' ); ?></h4>
		<?php do_action( 'bp_before_profile__upload_content' ); ?>

			<p><?php _e( 'Du kan laste opp et bilde fra din disk.', 'buddypress'); ?></p>
			...
			<form action="" method="post" id="cover-upload-form" class="standard-form" enctype="multipart/form-data">
				<?php wp_nonce_field( 'bp_cover_upload' ); ?>
					<p><?php _e( 'Klikk nedenfor for &aring; laste opp et bilde i .jpg eller .png format ', 'buddypress' ); ?></p>
				...
				<!--		<input type="number" name="position_x" style="width:80px;" value="<?php /*echo (isset($position_x)?$position_x:'50%'); ?>" placeholder="<?php esc_attr_e( 'X Postion %', 'buddypress' ); ?>" />
						...
						<input type="submit" name="profile_cover" id="upload" value="<?php esc_attr_e( 'Last opp', 'buddypress' ); ?>" />
					</p><br />
					
					<?php if ( bp_get_user_has_cover() ) : ?>
						<p><?php _e( "&Oslash;nsker du &aring; slette coverbildet uten &aring; laste opp et nytt? Klikk p&aring; slett knappen nedefor.", 'buddypress' ); ?></p>
						<input type="submit" name="delete_profile_cover" id="delete" value="<?php esc_attr_e( 'Slett', 'buddypress' ); ?>" />
					<?php endif; ?>
	...
	function bp_group_profile_cover(){ 
       ...
       if(!isset($position_y)){
        ...
        echo '<style> #item-header{
        	padding:20px;    
        	background:url("'.$cover_url.'") '.$position_x.'% '.$position_y.'% '.$repeat.' fixed !important;
