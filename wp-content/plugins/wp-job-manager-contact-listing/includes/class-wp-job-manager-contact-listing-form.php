...
class Astoundify_Job_Manager_Contact_Listing_Form extends Astoundify_Job_Manager_Contact_Listing {
	...
	public function job_manager_application_details_email( $apply ) {
		...
		//subscriber vil si at kun artister kan søke
		if ( current_user_can( 'subscriber') ){
		do_action( 'job_manager_contact_listing_form_' . $plugin, $form );
		}else if( current_user_can( 'administrator') ){
			do_action( 'job_manager_contact_listing_form_' . $plugin, $form );
		}else{
			echo('<span><p>Beklager, Kun artister kan sende inn tilbud. </p></span>');
		}

		$user_ID = get_current_user_id();
 		$telefonummerFraBruker = get_user_meta($user_ID, 'phone', true);
		echo '<span id="tlfFraBrukerSpan" style="display: none;">' . $telefonummerFraBruker .'</span>';
	}
